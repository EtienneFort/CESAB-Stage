# Utilisation code Arnaud pour espace fonctionnel, EF, 22/03/2023
...
# hist(sp_traits$habitat)
# hist(sp_traits$spawning.type)
# hist(sp_traits$feeding.mode)
...
#########################
#passage des traits quantitatifs en log(x+1)
load("Dataset/Output/fonctio/Table_species_traits.Rdata")
sp_traits_log = sp_traits
sp_traits_log[,-(1:3)] = log(sp_traits[,-(1:3)]+1)

gow.matrix.log <- daisy(sp_traits_log, metric = "gower")
gow.pco.log <- pco(gow.matrix.log,k=6)
save(gow.pco.log, file = file.path(filepath_save,"Gower_PCOA_log.Rdata"))
load("Dataset/Output/fonctio/Gower_PCOA_log.Rdata")

PCOA1<-data.frame(gow.pco.log$points[,1])[,1]
PCOA2<-data.frame(gow.pco.log$points[,2])[,1]
PCOA3<-data.frame(gow.pco.log$points[,3])[,1]
PCOA4<-data.frame(gow.pco.log$points[,4])[,1]
PCOA5<-data.frame(gow.pco.log$points[,5])[,1]
PCOA6<-data.frame(gow.pco.log$points[,6])[,1]



# Overlay significant traits (https://www.rdocumentation.org/packages/vegan/versions/2.6-4/topics/envfit)
gow.env.log <- envfit(gow.pco.log, sp_traits_log, permutations = 999, na.rm = TRUE)
save(gow.env.log, file = file.path(filepath_save,"Gower_env_log.Rdata"))

plot(PCOA1,PCOA2,pch=19, cex = 0.3);abline(v=0,h=0);plot(gow.env.log, p.max = 0.05, cex = 0.6)
